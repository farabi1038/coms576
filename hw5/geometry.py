...
def get_nearest_point_on_dubins_line(dubins_edge, p, tol=1e-3):

    if len(dubins_edge) == 0:
        return (None, None)

    min_dis = math.inf
    min_points = dubins_edge[0]
    point1 = np.array(p)
    for point in dubins_edge:
        point2 = np.array(point)
        # dist = np.linalg.norm(point1 - point2)
        dist = get_euclidean_distance(point1, point2)
        if min_dis > dist:
            min_dis = dist
            min_points = point2
    return (min_points, min_dis)
