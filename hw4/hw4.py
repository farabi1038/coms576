...
class RRT:
    ...
    def planning(self, animation=True):
        ...
        """
        #take a radom node and find the nearest
    ...
    def planning3(self,k,N,animation=True):
        ...
        animation: flag for animation
        """
        main_prm(start=[self.start.x,self.start.y],goal=[self.end.x,self.end.y],O=self.obstacle_list,rr=self.robot_radius,dt=self.dt)
...
if __name__ == '__main__':
    ...
    #path = rrt.planning2(animation=show_animation)
        ...
        rrt = RRT(start=xI,goal=XG,rand_area=C,obstacle_list=obstacleList,robot_radius=0.01)
        path = rrt.planning(animation=show_animation)
    elif args.prob=='2':
        rrt = RRT(start=xI,goal=XG,rand_area=C,obstacle_list=obstacleList,robot_radius=0.01)
        path = rrt.planning2(animation=show_animation)
    elif args.prob=='3':
        rrt = RRT(start=xI,goal=XG,rand_area=C,obstacle_list=obstacleList,robot_radius=1)
        rrt.planning3(15,100,animation=show_animation)
    else : 
        rrt = RRT(start=xI,goal=XG,rand_area=C,obstacle_list=[],robot_radius=0.001)
        path = rrt.planning(animation=show_animation)
